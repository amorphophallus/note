...
3. 算术功能模块
    - 第三章待办（记得把之前的几个模块的原理图和输入输出图给贴上去）
...
5. Flip-Flop Input and Output Equation Determination
    ...
    - 无效状态的处理方法：
        1. 给无效状态赋合理的输出，消除跳转到无效状态的危害
        2. 添加一个额外的输出或使用不使用的输出，来标识进入额外状态
        3. 给无效状态定义合适的下一状态，使得无效状态可以回到有效状态
...
1. 共性与个性：![Alt text](./img/ROM_PAL_PLA.png)
2. ROM
...
3. SRAM
    - 结构：  ![Alt text](./img/SRAM.png)
        ...
        2. Bit Slice：复制粘贴 $2^N$ 个 RAM cell 来拓展地址
