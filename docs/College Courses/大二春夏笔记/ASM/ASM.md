...
### 从 70h & 71h 端口读取时间数据 readtime.asm
...
### 通过 60h 端口访问键盘 + 自定义 int 9h 中断函数：key.asm
...
本来的 int 00h 返回之后会继续执行 div 指令，这里的修改使得中断处理函数返回之后跳过 div 指令直接执行下一条指令
...
main:
   ...
   ;
   mov ah, 1
   int 80h		; int 80h 的 1 号功能，魔改为 int 21h 的 1 号功能
next:
   mov ah, 2	; int 80h 的 2 号功能，魔改为输出 dl 中的字符
   mov dl, al
   int 80h
   ;
   mov ax, old_80h[0]
...
is_2:
   ...
   jmp done
is_1:
   int 21h		; 调用 int 21h 的 1 号功能，读取一个字符
done:
   iret
code ends
end main
...
- int 8h 和 int 9h 中断结束之后需要 out 来告诉操作系统中断结束，int 21h 就不需要
...
main:
   ...
   mov es:[bx+2], cs; 或 seg int8h 或 code
   sti; IF=1允许中断
wait_a_while:
   cmp stop, 1
   jne wait_a_while
